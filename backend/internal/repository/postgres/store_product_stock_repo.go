...
func (r *StoreRepo) Create(ctx context.Context, s *domain.Store) (*domain.Store, error) {
	const q = `
		INSERT INTO stores (name, address, phone, tax_number, currency)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id, name, address, phone, tax_number, currency, is_active, created_at, updated_at, deleted_at`
	row := &domain.Store{}
	err := r.db.QueryRowxContext(ctx, q, s.Name, s.Address, s.Phone, s.TaxNumber, s.Currency).StructScan(row)
	if err != nil {
...
func (r *StoreRepo) FindAll(ctx context.Context, filter dto.StoreListFilter) ([]*domain.Store, int, error) {
	...
	dataQ := fmt.Sprintf(`
		SELECT id, name, address, phone, tax_number, currency, is_active, created_at, updated_at, deleted_at
		FROM stores s %s
...
func (r *StoreRepo) Update(ctx context.Context, s *domain.Store) (*domain.Store, error) {
	const q = `
		UPDATE stores
		SET name=$1, address=$2, phone=$3, tax_number=$4, currency=$5, is_active=$6, updated_at=NOW()
		WHERE id=$7 AND deleted_at IS NULL
		RETURNING id, name, address, phone, tax_number, currency, is_active, created_at, updated_at, deleted_at`
	row := &domain.Store{}
	err := r.db.QueryRowxContext(ctx, q, s.Name, s.Address, s.Phone, s.TaxNumber, s.Currency, s.IsActive, s.ID).StructScan(row)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, nil
		}
