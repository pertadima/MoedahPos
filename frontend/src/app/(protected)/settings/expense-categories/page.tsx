...
function FormModal({ initial, onSuccess, onClose }: FormModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit && initial) {
        await expensesApi.updateCategory(initial.id, form);
      } else {
        await expensesApi.createCategory(form);
      }
      onSuccess();
    } catch (e) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="input-group">
            ...
            <input
              ...
              value={form.name}
              onChange={set('name')}
              placeholder="Contoh: Operasional, Listrik"
              autoFocus
            />
          </div>
          <div className="input-group">
      ...
      >
        <div>
          <h1 className="page-title" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Tag size={22} style={{ color: 'var(--accent-em)' }} /> Kategori Pengeluaran
          </h1>
          <p className="page-subtitle">Kelola kategori global untuk pengeluaran seluruh toko</p>
        </div>
