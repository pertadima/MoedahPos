      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 24, borderBottom: '1px solid var(--border)' }}>
        ...
        <button
          ...
          style={{
            ...
            borderBottom: activeTab === 'rutin' ? '2px solid var(--primary)' : '2px solid transparent',
            cursor: 'pointer'
          }}
        >
          Pengeluaran Rutin
              ...
              <tbody>
                {expenses.map(exp => (
                  <tr key={exp.id}>
                    ...
                    <td style={{ textAlign: 'right', gap: 8, display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
                      {exp.payment_status === 'unpaid' && (
                        <button
    ...
    <>
      ...
      {loading && routines.length === 0 ? (
        ...
        <div style={{
          ...
          <h3 style={{ margin: '0 0 8px', fontSize: '1.1rem', color: 'var(--text-1)' }}>Belum Ada Pengeluaran Rutin</h3>
          <p style={{ margin: 0, color: 'var(--text-3)', fontSize: '0.9rem' }}>
                ...
                {routines.map(rt => (
                  <tr key={rt.id}>
                    <td>
                      ...
                      <div style={{ fontSize: '0.8rem', color: 'var(--text-3)' }}>Mulai: {formatDate(rt.start_date)}</div>
                    </td>
                    <td>
                      <span style={{ 
                        background: 'var(--bg-hover)', 
                    ...
                    <td style={{ color: 'var(--text-1)', fontWeight: 500 }}>{formatDate(rt.next_run_date)}</td>
                    <td>
                      {rt.is_active ? (
                        <span style={{ color: '#16a34a', fontSize: '0.85rem', fontWeight: 600 }}>Aktif</span>
                      ) : (
                        <span style={{ color: '#9ca3af', fontSize: '0.85rem', fontWeight: 600 }}>Tidak Aktif</span>
                      )}
                    </td>
                    <td style={{ textAlign: 'right', fontWeight: 600, color: '#dc2626' }}>
                      {formatRp(rt.amount)}
...
function RecurringExpenseModal({ 
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--bg-card)', padding: 24, borderRadius: 12, width: 500, maxWidth: '90%', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h2 style={{ margin: 0, fontSize: '1.2rem' }}>{expense ? 'Ubah Pengeluaran Rutin' : 'Buat Templat Rutin'}</h2>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)' }}>
            <X size={20} />
          </button>
        </div>
        
        {err && <div style={{ background: '#fee2e2', color: '#b91c1c', padding: 12, borderRadius: 6, fontSize: '0.85rem', marginBottom: 16 }}>{err}</div>}
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {expense && (
            <div>
              ...
              <p style={{ margin: '4px 0 0', fontSize: '0.8rem', color: 'var(--text-3)' }}>Jika tidak aktif, sistem tidak akan membuat tagihan ini secara otomatis.</p>
            </div>
          )}
