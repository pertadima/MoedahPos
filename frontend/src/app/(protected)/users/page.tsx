...
import {
  Users, Plus, Loader2, X, Search, Shield,
...
function Avatar({ name, size = 32 }: { name: string; size?: number }) {
  ...
  return (
    <div style={{
      ...
      fontWeight: 800, fontSize: size * 0.34, color: '#fff', flexShrink: 0,
    }}>{initials}</div>
  );
}
...
function StoreAssigner({ value, onChange, roles, stores }: {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <span style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-2)' }}>Penugasan Toko</span>
        <button type="button" className="btn btn-secondary btn-sm" onClick={add} style={{ fontSize: '0.72rem' }}>
          <Plus size={11} /> Tambah
        </button>
      </div>
      {value.length === 0 && (
        <div style={{ padding: '10px 12px', border: '1px dashed var(--border)', borderRadius: 8, fontSize: '0.8rem', color: 'var(--text-3)', textAlign: 'center' }}>
          Belum ada penugasan
        </div>
      )}
      {value.map((a, i) => (
        <div key={i} style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'center' }}>
          <select className="input" style={{ flex: 2, fontSize: '0.8rem' }} value={a.store_id} onChange={e => update(i, 'store_id', e.target.value)}>
            {stores.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
          </select>
          <select className="input" style={{ flex: 1, fontSize: '0.8rem' }} value={a.role_id} onChange={e => update(i, 'role_id', e.target.value)}>
            {roles.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
          </select>
          <button type="button" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', padding: 4 }} onClick={() => remove(i)}>
            <X size={13} />
          </button>
        </div>
      ))}
...
function UserFormModal({ mode, user, roles, stores, onSuccess, onClose }: {
  mode: FormMode;
  user?: UserAdmin;
  roles: Role[];
  stores: { id: string; name: string }[];
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'create' && !form.password) { setError('Password wajib diisi'); return; }

    setLoading(true);
    try {
      if (mode === 'create') {
        const created = await usersAdminApi.create({ name: form.name, email: form.email, password: form.password, stores: form.stores }) as any;
...
function ResetPasswordModal({ user, onSuccess, onClose }: { user: UserAdmin; onSuccess: () => void; onClose: () => void }) {
  const [pw, setPw] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const handleReset = async () => {
    if (!pw || pw.length < 6) { setError('Minimal 6 karakter'); return; }
    setLoading(true);
    try {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" style={{ maxWidth: 380 }} onClick={e => e.stopPropagation()}>
        ...
        {error && <div style={{ background: 'rgba(239,68,68,0.1)', borderRadius: 8, padding: '8px 12px', color: '#f87171', fontSize: '0.82rem', marginBottom: 12 }}>{error}</div>}
...
// ── Deactivate Confirm ────────────────────────────────────────────────────────
function DeactivateConfirm({ user, onSuccess, onClose }: { user: UserAdmin; onSuccess: () => void; onClose: () => void }) {
  const [loading, setLoading] = useState(false);
  const handle = async () => {
    setLoading(true);
    try { await usersAdminApi.deactivate(user.id); onSuccess(); }
      ...
      <div className="modal-box" style={{ maxWidth: 400 }} onClick={e => e.stopPropagation()}>
        <div style={{ textAlign: 'center', padding: '8px 0 16px' }}>
          ...
          <p style={{ color: 'var(--text-2)', fontSize: '0.875rem', lineHeight: 1.6 }}>
            <strong>{user.name}</strong> akan diarsipkan dan tidak dapat login. Data dan riwayat transaksi tetap tersimpan.
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-secondary" style={{ flex: 1 }} onClick={onClose} disabled={loading}>Batal</button>
...
function DetailDrawer({ user, roles: _roles, stores: _stores, onClose, onEdit, onDeactivate, onResetPw }: {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 420, background: 'var(--bg-card)', borderLeft: '1px solid var(--border)', zIndex: 201, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        {/* header */}
        <div style={{ padding: '18px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button className="btn btn-secondary btn-sm" onClick={onResetPw} title="Reset Password"><KeyRound size={13} /></button>
            {user.is_active && (
              <button style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 6, border: '1px solid rgba(245,158,11,0.4)', background: 'rgba(245,158,11,0.1)', color: '#f59e0b', fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer' }} onClick={onDeactivate}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
          {/* profile */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: 'var(--bg-elevated)', borderRadius: 12, padding: '16px', marginBottom: 16, border: '1px solid var(--border-md)' }}>
            <Avatar name={user.name} size={48} />
            <div>
              <div style={{ fontWeight: 800, fontSize: '1.05rem' }}>{user.name}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.8rem', color: 'var(--text-2)', marginTop: 2 }}>
                <Mail size={11} /> {user.email}
              </div>
              <div style={{ marginTop: 6 }}><StatusBadge active={user.is_active} /></div>
            </div>
          </div>

          {/* store memberships */}
          <div style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 4 }}>
            <Store size={11} /> Penugasan Toko ({user.store_count})
          </div>
          {(user.stores && user.stores.length > 0) ? user.stores.map((s: UserStoreAssignment, i: number) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: 'var(--bg-elevated)', borderRadius: 8, marginBottom: 6, border: '1px solid var(--border)' }}>
              <div>
          ...
          <div style={{ marginTop: 16, fontSize: '0.68rem', fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'flex', alignItems: 'center', gap: 4 }}>
            <Shield size={11} /> Peran
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
            {user.stores && user.stores.length > 0
              ? [...new Set(user.stores.map((s: UserStoreAssignment) => s.role_name))].map(r => <RoleBadge key={r} name={r} />)
              : <span style={{ fontSize: '0.78rem', color: 'var(--text-3)', fontStyle: 'italic' }}>—</span>}
          </div>

          {/* meta */}
          <div style={{ marginTop: 20, padding: '12px 14px', background: 'var(--bg-elevated)', borderRadius: 8, border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 4 }}>
            {[
              { label: 'Dibuat', val: new Date(user.created_at).toLocaleDateString('id-ID', { dateStyle: 'long' }) },
              { label: 'Diperbarui', val: new Date(user.updated_at).toLocaleDateString('id-ID', { dateStyle: 'long' }) },
            ].map(({ label, val }) => (
...
export default function UsersPage() {
  ...
  const load = useCallback(async (p = 1) => {
    ...
    setError('');
    try {
      const body = await usersAdminApi.list({ search, include_inactive: includeInactive, page: p, per_page: PER_PAGE }) as any;
      setUsers(body.data ?? []);
  ...
  useEffect(() => {
    rolesApi.list().then(body => { const d = body as any; setRoles(d.data ?? []); }).catch(() => {});
    storesApi.list().then(body => {
  ...
  const refresh = () => { load(page); if (detail) openDetail(detail); };
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.83rem', cursor: 'pointer', userSelect: 'none' }}>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        {loading ? (
          ...
          <table className="tbl">
            ...
            <tbody>
              {users.map(u => (
                <tr key={u.id} style={{ cursor: 'pointer', opacity: u.is_active ? 1 : 0.6 }} onClick={() => openDetail(u)}>
                  <td>
                  ...
                  <td>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: '0.78rem', color: 'var(--text-2)' }}>
                  ...
                  <td onClick={e => e.stopPropagation()}>
                    <div style={{ display: 'flex', gap: 4 }}>
                      ...
                      <button className="btn btn-ghost btn-sm" onClick={() => openDetail(u)}><ChevronRight size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 14 }}>
          <span style={{ fontSize: '0.8rem', color: 'var(--text-3)' }}>Hal {page} dari {totalPages} · {total} pengguna</span>
          <div style={{ display: 'flex', gap: 6 }}>
            <button className="btn btn-secondary btn-sm" disabled={page <= 1} onClick={() => load(page - 1)}>‹</button>
            <button className="btn btn-secondary btn-sm" disabled={page >= totalPages} onClick={() => load(page + 1)}>›</button>
          </div>
        </div>
      )}
      ...
      {/* ── Modals ── */}
      {createOpen && (
        <UserFormModal mode="create" roles={roles} stores={allStores} onSuccess={() => { setCreateOpen(false); load(1); }} onClose={() => setCreateOpen(false)} />
      )}
      {editUser && (
        <UserFormModal mode="edit" user={editUser} roles={roles} stores={allStores} onSuccess={() => { setEditUser(null); refresh(); }} onClose={() => setEditUser(null)} />
      )}
      {deactivating && (
        <DeactivateConfirm user={deactivating} onSuccess={() => { setDeactivating(null); load(page); if (detail?.id === deactivating.id) setDetail(null); }} onClose={() => setDeactivating(null)} />
      )}
      {resetting && (
        <ResetPasswordModal user={resetting} onSuccess={() => setResetting(null)} onClose={() => setResetting(null)} />
      )}
      {detail && (
        <DetailDrawer
          user={detail} roles={roles} stores={allStores}
          onClose={() => setDetail(null)}
          onEdit={() => { setEditUser(detail); setDetail(null); }}
          onDeactivate={() => { setDeactivating(detail); setDetail(null); }}
