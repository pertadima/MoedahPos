...
import {
  Store, Plus, Pencil, Trash2, Check, X, Loader2,
...
const emptyForm = (): StoreForm => ({
  ...
  currency: 'IDR', store_type: 'retail',
});

const STORE_TYPE_CONFIG = {
  retail:     { label: 'Retail',     icon: ShoppingBag,     color: '#10b981', bg: 'rgba(16,185,129,0.12)' },
  restaurant: { label: 'Restaurant', icon: UtensilsCrossed, color: '#fb923c', bg: 'rgba(251,146,60,0.12)'  },
};
...
export default function StoresPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; mode: 'create' | 'edit'; store?: StoreType }>({ open: false, mode: 'create' });
  const [deleteConfirm, setDeleteConfirm] = useState<{ open: boolean; store?: StoreType }>({ open: false });
  ...
  const openEdit = (s: StoreType) => {
    setForm({
      ...
      tax_number: s.tax_number, currency: s.currency,
    ...
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
          {stores.map(store => {
            const typeCfg = STORE_TYPE_CONFIG[store.store_type as 'retail' | 'restaurant'] ?? STORE_TYPE_CONFIG.retail;
            const TypeIcon = typeCfg.icon;
            return (
              <div key={store.id} className="card" style={{ padding: 0, overflow: 'hidden', opacity: store.is_active ? 1 : 0.65 }}>
                {/* Top accent strip */}
                <div style={{ height: 3, background: store.is_active ? typeCfg.color : 'var(--border)' }} />
                ...
                <div style={{ padding: '16px 18px 14px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 700, fontSize: '0.92rem', color: 'var(--text-1)', lineHeight: 1.2 }}>{store.name}</div>
                        <div style={{
                          display: 'inline-flex', alignItems: 'center', gap: 3,
                          marginTop: 3, fontSize: '0.67rem', padding: '1px 6px',
                    ...
                    <div style={{
                      ...
                      background: store.is_active ? 'rgba(16,185,129,0.12)' : 'rgba(239,68,68,0.10)',
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 12 }}>
                    {store.address && (
                      <div style={{ display: 'flex', gap: 6, alignItems: 'flex-start', fontSize: '0.78rem', color: 'var(--text-2)' }}>
                        <MapPin size={12} style={{ color: 'var(--text-3)', marginTop: 1, flexShrink: 0 }} />
                        <span style={{ lineHeight: 1.4 }}>{store.address}</span>
                      </div>
                    )}
                    {store.phone && (
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: '0.78rem', color: 'var(--text-2)' }}>
      ...
      {modal.open && (
        <div style={{
          ...
          <div className="card" style={{ width: '100%', maxWidth: 520, padding: 28, animation: 'slideIn 0.2s ease' }}>
            {/* Modal header */}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-3)' }}>
                    {modal.mode === 'create' ? 'Isi informasi toko baru' : 'Perbarui informasi toko'}
                  </div>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: 18 }}>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, marginBottom: 8, color: 'var(--text-2)' }}>
                  ...
                  {(['retail', 'restaurant'] as const).map(t => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {active && <Check size={14} style={{ marginLeft: 'auto', flexShrink: 0 }} />}
                      </button>
                    );
                  })}
                </div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input className="input" placeholder="cth. Warung Kopi Makmur" autoFocus
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <textarea className="input" rows={2} placeholder="Jl. Contoh No. 1, Jakarta"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                <div>
                  ...
                  <input className="input" placeholder="021-12345678"
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, marginBottom: 5, color: 'var(--text-2)' }}>
                  NPWP <span style={{ fontSize: '0.72rem', color: 'var(--text-3)', fontWeight: 400 }}>(opsional)</span>
                </label>
                <input className="input" placeholder="00.000.000.0-000.000"
                  value={form.tax_number} onChange={e => setForm(f => ({ ...f, tax_number: e.target.value }))} />
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={submitting} style={{ gap: 8 }}>
                  {submitting
                    ...
                    : <><Check size={14} /> {modal.mode === 'create' ? 'Buat Toko' : 'Simpan Perubahan'}</>
                  }
                </button>
              </div>
            </form>
      ...
      {deleteConfirm.open && deleteConfirm.store && (
        <div style={{
          ...
          <div className="card" style={{ width: '100%', maxWidth: 400, padding: 28, textAlign: 'center', animation: 'slideIn 0.2s ease' }}>
            <div style={{
            ...
            <div style={{
              ...
              borderRadius: 8, padding: '8px 12px', marginBottom: 20, textAlign: 'left',
            }}>
              ⚠️ Data produk, transaksi, dan anggota toko ini tidak akan terhapus — hanya toko yang tersembunyi.
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                ...
                style={{ flex: 1, gap: 8, background: 'rgba(239,68,68,0.15)', color: '#ef4444', border: '1px solid rgba(239,68,68,0.3)' }}
              >
