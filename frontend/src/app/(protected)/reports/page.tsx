...
const CUSTOM_TOOLTIP_STYLE = {
  ...
  borderRadius: 8, color: 'var(--text-1)', fontSize: 12,
};
  ...
  if (!selectedStore) return (
    <div style={{ padding: 32 }}>
      <div className="empty-state card" style={{ padding: 40 }}>
        <BarChart3 size={40} /><p>Pilih toko terlebih dahulu</p>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
        ...
        <div className="stat-card">
          <div className="stat-icon" style={{ background: 'rgba(239,68,68,0.12)' }}><TrendingDown size={20} style={{ color: '#ef4444' }} /></div>
          <div>
            <div className="stat-label">Total HPP</div>
            <div className="stat-val" style={{ color: '#ef4444' }}>{formatRp((summary as any)?.total_cost ?? 0)}</div>
          </div>
        </div>
        <div className="stat-card">
          <div className="stat-icon" style={{ background: 'rgba(245,158,11,0.12)' }}><Activity size={20} style={{ color: '#f59e0b' }} /></div>
          <div>
            <div className="stat-label">Gross Profit</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2 }}>
              <div className="stat-val" style={{ color: '#f59e0b' }}>{formatRp((summary as any)?.gross_profit ?? 0)}</div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={chartData}>
                  ...
                  <Tooltip contentStyle={CUSTOM_TOOLTIP_STYLE} formatter={(v: unknown) => [formatRp(Number(v)), 'Penjualan']} />
          ...
          <div className="card" style={{ overflow: 'hidden' }}>
            <table className="tbl">
              <thead><tr><th>Tanggal</th><th>Transaksi</th><th>Penjualan</th><th>HPP</th><th>Profit</th><th>Margin</th><th>Pajak</th><th>Diskon</th></tr></thead>
              <tbody>
                {(summary?.rows ?? []).map(r => (
                  <tr key={r.date}>
                    ...
                    <td style={{ fontWeight: 700, color: '#f59e0b' }}>{formatRp((r as any).gross_profit ?? 0)}</td>
                    <td><ProfitMarginBadge margin={(r as any).profit_margin ?? 0} /></td>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          <table className="tbl">
            <thead><tr><th>Produk</th><th>SKU</th><th>Qty</th><th>Revenue</th><th>HPP</th><th>Profit</th><th>Margin</th><th>Pajak</th></tr></thead>
            <tbody>
              {byProduct.map((r, i) => (
                <tr key={r.product_id || i}>
                  <td><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ background: 'var(--bg-elevated)', width: 24, height: 24, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-2)' }}>{i+1}</span>
                    {r.product_name}
                  </div></td>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 10 }}>
              {[
                { label: 'Total Revenue', val: formatRp(profit?.total_sales ?? 0), color: '#10b981' },
                { label: 'Total HPP', val: formatRp(profit?.total_cost ?? 0), color: '#ef4444' },
                { label: 'Gross Profit', val: formatRp(profit?.gross_profit ?? 0), color: '#f59e0b' },
              ].map(({ label, val, color }) => (
            ...
            {profitChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height={260}>
                <ComposedChart data={profitChartData}>
                  ...
                  <YAxis yAxisId="left" tick={{ fill: 'var(--text-3)', fontSize: 11 }} axisLine={false} tickLine={false}
                    tickFormatter={v => v >= 1000000 ? `${(v/1000000).toFixed(1)}M` : `${(v/1000).toFixed(0)}K`} />
                  <YAxis yAxisId="right" orientation="right" tick={{ fill: 'var(--text-3)', fontSize: 11 }} axisLine={false} tickLine={false}
                    tickFormatter={v => `${v}%`} domain={[0, 100]} />
                  <Tooltip contentStyle={CUSTOM_TOOLTIP_STYLE}
                    formatter={(v: unknown, name: string) =>
                      name === 'margin'
                        ? [`${Number(v).toFixed(1)}%`, 'Margin']
                        : [formatRp(Number(v)), name === 'revenue' ? 'Revenue' : name === 'cost' ? 'HPP' : 'Profit']
                    }
                  />
                  <Area yAxisId="left" type="monotone" dataKey="revenue" fill="rgba(99,102,241,0.1)" stroke="#6366f1" strokeWidth={2} name="revenue" />
                  <Area yAxisId="left" type="monotone" dataKey="cost" fill="rgba(239,68,68,0.1)" stroke="#ef4444" strokeWidth={2} name="cost" />
                  <Bar yAxisId="left" dataKey="profit" fill="rgba(245,158,11,0.8)" radius={[4,4,0,0]} name="profit" />
                  <Line yAxisId="right" type="monotone" dataKey="margin" stroke="#10b981" strokeWidth={2} dot={false} name="margin" />
                </ComposedChart>
              </ResponsiveContainer>
            ) : <div className="empty-state" style={{ height: 200 }}><BarChart3 size={32} /><p>Belum ada data profit</p></div>}
          </div>
              ...
              <tbody>
                {(profit?.rows ?? []).map((r: any) => (
                  <tr key={r.period}>
                    ...
                    <td style={{ color: 'var(--accent-em)', fontWeight: 600 }}>{formatRp(r.total_sales)}</td>
                    <td style={{ color: '#ef4444' }}>{formatRp(r.total_cost)}</td>
                    <td style={{ fontWeight: 800, color: '#f59e0b' }}>{formatRp(r.gross_profit)}</td>
                    <td><ProfitMarginBadge margin={r.profit_margin} /></td>
                  </tr>
                ))}
                {profit && (
                  <tr style={{ background: 'var(--bg-elevated)', fontWeight: 700 }}>
                    ...
                    <td><ProfitMarginBadge margin={profit.profit_margin} /></td>
                  </tr>
                )}
              </tbody>
            </table>
